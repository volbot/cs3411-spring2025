				...
				if(conxs[i].state && FD_ISSET(conxs[i].fd,&fds)) {
					...
					if(bytes_to_read>0) {
						...
						if(conxs[i].state == CONX_STATE_CONNECTED){
							...
							int cmd_log_found = 0;
							//only check for log if 3 or more characters exist, because the way
							//i'm making a substring has potential for a segfault if too short
							if(sizeof(readbuf)>=3){
								...
								if(strcmp(cmd_buf,"log")==0){
									...
									//get the complementary substring for logfile
									...
									//with a newline for block separation
